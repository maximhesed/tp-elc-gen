...
def els_place(canvas, elq, dx, dy, ly):
    els = list()
    lbls = list()
    ind = 0
    ...
    for i in range(elq):
        ...
        els[i] = canvas.create_rectangle(dx * ind - dx, ly - dy,
            dx * (ind + 1) - dx, ly, outline="#000", fill="#1c1", width=2)
        lbls[i] = canvas.create_text(dx * (ind + 0.5) - dx, ly - dy / 2,
...
def man_init(canvas, r, dx, dy):
    ...
    v = canvas.create_oval([r / 2, r / 2], [r, r], fill="blue")
...
def main():
    ...
    els, lbls = els_place(canvas, elq, dx, dy, ly) # place elements
